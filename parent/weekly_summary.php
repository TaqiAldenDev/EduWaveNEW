...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['child_id'])) {
    ...
    $grades_stmt = $pdo->prepare('SELECT subjects.name as subject_name, grades.exam_type, grades.score FROM grades JOIN subjects ON grades.subject_id = subjects.id WHERE grades.student_id = ? AND grades.date_given >= ?');
    $grades_stmt->execute([$child_id, $seven_days_ago]);
    $grades = $grades_stmt->fetchAll();

    $attendance_stmt = $pdo->prepare('SELECT subjects.name as subject_name, attendance.attend_date, attendance.status FROM attendance JOIN subjects ON attendance.subject_id = subjects.id WHERE attendance.student_id = ? AND attendance.attend_date >= ?');
    $attendance_stmt->execute([$child_id, $seven_days_ago]);
...
<body>
                        ...
                        <div class="card">
                            <div class="card-body">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-select" id="child_id" name="child_id" onchange="this.form.submit()">
                                                ...
                                                <?php foreach ($children as $child): ?>
                                                    <option value="<?= $child['id'] ?>" <?= (isset($_GET['child_id']) && $_GET['child_id'] == $child['id']) ? 'selected' : '' ?>><?= htmlspecialchars($child['name']) ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <?php if (isset($_GET['child_id'])): ?>
                                    ...
                                    $stmt->execute([$_GET['child_id']]);
                                    $child = $stmt->fetch();
                                    ?>
                                    <h4>Summary for <?= htmlspecialchars($child['name']) ?></h4>

                                    <?php if (!empty($grades)): ?>
                                        <div class="card mt-3">
                                            <div class="card-header">
                                                <h5>Grades (Last 7 Days)</h5>
                                            </div>
                                            <div class="card-body">
    ...
    <script>
        // For the select onchange to submit the form
