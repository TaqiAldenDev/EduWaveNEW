...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['role'])) {
        ...
        try {
            $stmt = $pdo->prepare('UPDATE users SET name = ?, email = ?, role = ?, color_theme = ? WHERE id = ?');
            $stmt->execute([$name, $email, $role, $color_theme, $user_id]);
...
<head>
    ...
    <title>Edit User - EduWave Admin</title>
