...
<body>
    ...
    <script>
        function exportTable(type) {
            const fileName = 'parents_children_' + new Date().toISOString().slice(0, 10);
            
            const options = {
                tableName: 'Parents and Their Children',
                worksheetName: 'Parents',
                fileName: fileName,
        ...
        switch(type) {
            case 'xlsx':
                options.type = 'xlsx';
                $('#parentsTable').tableExport(options);
                break;
            case 'csv':
                exportToCSV();
                break;
